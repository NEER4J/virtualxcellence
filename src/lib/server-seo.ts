...
/**
 * Fetch SEO data from database based on page path (Server-side)
 */
export async function getServerSeoData(pagePath: string): Promise<SeoData | null> {
  try {
...
/**
 * Generate Next.js metadata from SEO data (Server-side)
 */
export function generateServerMetadata(seoData: SeoData | null, fallbackData?: Partial<SeoData>): Metadata {
...
/**
 * Generate server-side metadata for any page
...
export async function generatePageSeoMetadata(
  ...
  const seoData = await getServerSeoData(pagePath)
  return generateServerMetadata(seoData, fallbackData)
}
